...
import com.util.Article;
import com.util.MsgRequest;
import com.util.MsgResponseNews;
import com.util.MsgResponseText;
...
public class WxApiServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("timestamp") == null){//如果是空打印出首页
			response.getWriter().write(" hello jeeweixin (www.weixinpy.com) ");
		}else{
			...
			if (validSign(signature, prop.getProperty("token"), timestamp, nonce)) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		logger.info("##### process msg  ");
		try {
			MsgRequest msgRequest = MsgXmlUtil.parseXml(request);//获取发送的消息
			if("event".equals(msgRequest.getMsgType())){//订阅消息(subscribe)，回复一条图文消息
				...
				reponseText.setContent("欢迎您关注 程序员 coder10，请回复 1 ");
				
				String str = MsgXmlUtil.textToXml(reponseText);
				logger.info("##### response.write = " + str);
				
				response.getWriter().write(new String(str.getBytes("UTF-8"),"ISO8859-1"));
			}else{//否则返回图文消息
				...
				a1.setTitle("威阿发么哩 : 免费的微信开发者服务平台");
				a1.setPicUrl("http://h.gochehui.com/Public/images/hero-home.jpg");
				a1.setUrl("http://h.gochehui.com/");
				articles.add(a1);
				
				Article a2 = new Article();
				a2.setTitle("jeeweixin-微信开发者java源码");
				a2.setPicUrl("http://h.gochehui.com/Public/images/weixin.png");
				a1.setUrl("http://weixinpy.com/vp/source/download/");
				articles.add(a2);
				
				news.setArticleCount(articles.size());
				news.setArticles(articles);
				
				String str = MsgXmlUtil.newsToXml(news);
